...
export default function AppLayoutWrapper({ children }) {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col md:flex-row bg-surface">
      ...
      {/* SideNavBar */}
      <aside className={`fixed left-0 top-0 h-screen w-64 flex flex-col justify-between p-4 z-50 bg-surface-container border-r border-outline-variant transition-transform duration-300 ease-in-out ${
        ...
        <div className="p-3 bg-surface-container-low rounded-xl flex items-center gap-3 border border-outline-variant/40">
          ...
          <div className="overflow-hidden flex-1">
            <p className="text-sm font-semibold text-on-surface truncate">{user?.name || "Student"}</p>
            <p className="text-xs text-on-surface-variant truncate">{user?.email || "learner@recall.ai"}</p>
          </div>
        </div>
      </aside>
