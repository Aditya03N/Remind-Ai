...
export default function Dashboard() {
  ...
  const healthTrend = dashboardData?.healthTrend || 0;
  const activeConcepts = dashboardData?.activeConcepts || 0;
  const stats = dashboardData?.stats || { strong: 0, atRisk: 0, critical: 0, mastered: 0 };
  const immediateFocus = dashboardData?.immediateFocus;
    ...
    <>
        ...
        <section className="mb-6">
          <div className="bg-error-container text-on-error-container p-6 rounded-2xl shadow-lg border border-error/20 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-xl font-bold" style={{fontFamily: "var(--font-headline-sm)"}}>Manual Reminders Due!</h3>
                <p className="text-sm opacity-90 mt-1">You have {dueManualReminders.length} scheduled revision{dueManualReminders.length > 1 ? 's' : ''} waiting for you right now.</p>
              </div>
            </div>
            <div className="flex flex-col gap-2 w-full sm:w-auto">
              {dueManualReminders.map((reminder, idx) => (
                <Link key={idx} href={`/retention-check/${reminder.concept?._id}`} className="px-5 py-2.5 bg-error text-on-error hover:bg-error/90 text-sm font-medium rounded-xl shadow-md transition-all flex items-center justify-center gap-2">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-surface-container-lowest p-6 rounded-2xl shadow-[0_4px_20px_rgba(124,58,237,0.04)] border border-outline-variant/40 flex flex-col md:flex-row gap-8 items-center h-full">
          <div className="relative w-40 h-40 flex-shrink-0">
            <svg viewBox="0 0 100 100" className="w-full h-full transform -rotate-90">
              <circle cx="50" cy="50" r="40" fill="transparent" stroke="var(--outline-variant)" strokeWidth="8" opacity="0.2" />
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-3xl font-extrabold text-on-surface" style={{fontFamily: "var(--font-headline-lg)"}}>{Math.round(healthScore)}</span>
              <span className="text-xs font-bold text-primary tracking-wider uppercase">Score</span>
            </div>
          </div>
          
          <div className="flex-1 w-full h-48">
            <h3 className="text-sm font-bold text-on-surface-variant mb-4">7-Day Knowledge Retention Trend</h3>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={dashboardData?.healthTrend || []}>
        ...
        <div className="lg:col-span-2 grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="bg-surface-container-lowest p-5 rounded-xl shadow-[0_4px_20px_rgba(124,58,237,0.04)] border border-outline-variant/40 flex flex-col justify-between hover:border-primary/50 transition-all">
            ...
            <div className="mt-4">
              <span className="text-2xl font-bold text-on-surface" style={{fontFamily: "var(--font-headline-md)"}}>{stats.strong}</span>
          ...
          <div className="bg-surface-container-lowest p-5 rounded-xl shadow-[0_4px_20px_rgba(124,58,237,0.04)] border border-outline-variant/40 flex flex-col justify-between hover:border-amber-500/50 transition-all">
            ...
            <div className="mt-4">
              <span className="text-2xl font-bold text-on-surface" style={{fontFamily: "var(--font-headline-md)"}}>{stats.atRisk}</span>
          ...
          <div className="bg-surface-container-lowest p-5 rounded-xl shadow-[0_4px_20px_rgba(124,58,237,0.04)] border border-outline-variant/40 flex flex-col justify-between hover:border-error/50 transition-all">
            ...
            <div className="mt-4">
              <span className="text-2xl font-bold text-on-surface" style={{fontFamily: "var(--font-headline-md)"}}>{stats.critical}</span>
          ...
          <div className="bg-surface-container-lowest p-5 rounded-xl shadow-[0_4px_20px_rgba(124,58,237,0.04)] border border-outline-variant/40 flex flex-col justify-between hover:border-primary/50 transition-all">
            ...
            <div className="mt-4">
              <span className="text-2xl font-bold text-on-surface" style={{fontFamily: "var(--font-headline-md)"}}>{stats.mastered}</span>
          ...
          {immediateFocus ? (
            <>
              <div className="flex flex-col gap-2 max-w-2xl relative z-10">
                ...
                <h4 className="text-3xl font-bold text-on-surface mt-1" style={{fontFamily: "var(--font-headline-lg)"}}>{immediateFocus.concept?.name}</h4>
              ...
              <div className="relative z-10 w-full md:w-auto flex flex-col sm:flex-row gap-2">
                <Link href={`/retention-check/${immediateFocus.concept?._id}`} className="px-6 py-3.5 bg-primary hover:bg-primary-container text-on-primary text-sm font-medium rounded-xl shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2 group">
            ...
            <div className="flex flex-col gap-2 max-w-2xl relative z-10">
              <h4 className="text-xl font-bold text-on-surface mt-1">You're all caught up!</h4>
              <p className="text-base text-on-surface-variant">Add more concepts to track to continue your learning journey.</p>
              <Link href="/subjects" className="mt-2 inline-flex w-fit px-4 py-2 bg-primary text-on-primary rounded-lg text-sm font-medium">Add Concepts</Link>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="flex flex-col gap-4 pb-16">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-on-surface-variant">{revisionPlan.length} concepts queued</span>
        </div>
        <div className="flex flex-col gap-2">
          {revisionPlan.length > 0 ? revisionPlan.map((item, i) => (
            <div key={i} className="bg-surface-container-lowest p-4 md:p-5 rounded-xl border border-outline-variant/40 hover:border-primary/40 transition-all flex items-center justify-between gap-4 shadow-[0_2px_8px_rgba(0,0,0,0.02)]">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-lg bg-surface-container flex items-center justify-center text-primary">
                  <span className="material-symbols-outlined">bolt</span>
                </div>
                <div>
                  <h5 className="text-sm font-medium text-on-surface">{item.concept?.name}</h5>
                  <p className="text-xs text-on-surface-variant">{Math.round(item.estimatedRetention)}% retention</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <Link href={`/retention-check/${item.concept?._id}`} className="px-4 py-2 bg-surface-container-low hover:bg-primary hover:text-on-primary text-on-surface rounded-lg text-sm transition-all">
                  Review
                </Link>
              </div>
            </div>
